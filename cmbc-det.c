...
void addLayer(int nonce_,int val){
	u_int64_t out[8]={0};
	u_int32_t m[16]={0};
	...
	for(int i=0;i<5;i++){
		for(int j=0;j<12;j++){
			// chosen
			arr[tot_index][j]=((out[repl[i][j][0]]>>repl[i][j][1])&0xFF);
...
int main(){
	...
	for(int i=0;i<32;i++){
		int val = nondet_int();
		__CPROVER_assume(val>=startVal&&val<endVal);
		for(int j=0;j<12;j++){
			xors[j]=arr[val][j]^xors[j];
		}
		// __CPROVER_assume(chosen>=0&&chosen<1);
